...
///
/// 화자별 음성: 기기에 남/여 ja 보이스가 있으면 voice 전환,
/// 없으면 피치(남 0.75 / 여 1.12)로 구분한다.
class TtsService {
  TtsService._();
  ...
  Future<void> _scanVoices() async {
    ...
    try {
      ...
      for (final v in voices) {
        ...
        if (_maleVoice == null && lower.contains('male') && !lower.contains('female')) {
          _maleVoice = voice;
        }
  ...
  Future<void> speakAs(String text, {required String gender}) async {
    ...
    final pitch = voice != null ? 1.0 : (male ? 0.75 : 1.12);
    await _speakWith(text, voice, pitch);
  }

  Future<void> _speakWith(String text, Map<String, String>? voice, double pitch) async {
    await _ensureInit();
    await _tts.stop();
    if (voice != null) {
