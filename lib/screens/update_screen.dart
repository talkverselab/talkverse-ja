      ...
      body: ListView(
        ...
        children: [
          ...
          if (!Platform.isAndroid) ...[
            _Panel(
              title: tr('iPhone은 TestFlight로 업데이트'),
              color: cs.primary,
              lines: [
                tr('새 빌드는 TestFlight 앱에 자동으로 올라옵니다.'),
                tr('TestFlight 앱을 열어 「업데이트」를 눌러 주세요.'),
                tr('알림을 켜 두면 새 빌드가 올 때마다 바로 알려 줍니다.'),
              ],
            ),
            const SizedBox(height: 20),
            Text(
              '${tr('갤럭시(안드로이드)에서는 이 화면에서 APK를 바로 받아 설치하고,\n')}${tr('iPhone에서는 애플 정책상 TestFlight 앱이 그 역할을 맡습니다.')}',
              style: TextStyle(
                fontSize: 11.5,
                height: 1.5,
  ...
  Widget _actions(ColorScheme cs) {
    switch (_stage) {
      ...
      case _Stage.upToDate:
        return _Button(label: tr('다시 확인'), color: cs.secondary, onTap: _check);
    }
  }
}
