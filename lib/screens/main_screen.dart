...
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        children: [
          const Positioned.fill(child: SeigaihaPattern(opacity: 0.06)),
          SafeArea(
            child: ListView(
                ...
                Row(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: const Icon(Icons.person, color: AppColors.beni, size: 20),
                    ),
                  ],
                ),
    ...
    return InkWell(
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
              color: AppColors.sumi.withValues(alpha: 0.06),
              blurRadius: 6,
    ...
    for (final level in ['L1', 'L2', 'L3']) {
      ...
      for (var i = 0; i < metas.length; i++) {
        ...
        items.add(_LessonItem(
          '${level == 'L1' ? 'EP' : 'D'}${i + 1}',
          '${meta.emoji} ${meta.title}${total == 0 ? ' (준비 중)' : ''}',
          state,
          meta: meta,
          learned: learned,
          total: total,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.search, color: AppColors.sumi),
            onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => const KanjiDictionaryScreen()),
            ),
          ),
        ],
      ),
                ...
                return GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: selected ? AppColors.beni : AppColors.kin.withValues(alpha: 0.6),
                      ),
                    ),
                    ...
                    child: Text(
                      f,
                      style: TextStyle(
                        color: selected ? AppColors.washi : AppColors.sumi,
  ...
  Widget build(BuildContext context) {
    ...
    switch (lesson.state) {
      case _LessonState.done:
        ...
        trailing = const Icon(Icons.check_circle, color: AppColors.matcha);
        break;
      case _LessonState.current:
        bg = AppColors.beni;
        border = AppColors.beniDeep;
        textColor = AppColors.washi;
        trailing = const Icon(Icons.arrow_forward, color: AppColors.washi);
        break;
      case _LessonState.locked:
        bg = AppColors.washiDeep;
        border = AppColors.kin.withValues(alpha: 0.3);
        textColor = AppColors.sumiLight;
        trailing = const Icon(Icons.lock, color: AppColors.sumiLight, size: 18);
        break;
    }
    return InkWell(
      onTap: lesson.state == _LessonState.locked || lesson.meta == null || lesson.total == 0
          ? null
          : () async {
              await Navigator.push(
                context,
