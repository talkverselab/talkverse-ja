...
/// 회화 문장 플래시카드.
/// - 기본: 한국어 앞면 → 뒤집으면 일본어 (방향 토글 가능, 설정 저장)
/// - 한국어 앞면에는 가나(읽기) 힌트 표시 (힌트 토글 가능, 설정 저장)
...
class _SentenceFlashcardScreenState extends State<SentenceFlashcardScreen> {
  ...
  /// 한국어 면 힌트 — 가나 읽기 앞부분만 (첫 6자 + …)
  String _hint(TurnRow t) {
    final k = t.kana ?? t.ja;
    final chars = k.characters.toList();
    if (chars.length <= 6) return k;
          ...
          children: [
            Text(
              ...
              style: const TextStyle(
                  color: AppColors.sumi, fontSize: 16, fontWeight: FontWeight.w800),
            ),
            const SizedBox(height: 2),
            Text(
              meta == null ? tr('전 레벨 랜덤 20') : trf('{0} 회화', [meta.level]),
              style: TextStyle(
                  color: AppColors.sumiLight, fontSize: 10, letterSpacing: 2),
            ),
          ],
        ),
        actions: [
          ...
          IconButton(
            tooltip: _koFirst ? tr('한국어 먼저 (탭: 일본어 먼저)') : tr('일본어 먼저 (탭: 한국어 먼저)'),
            onPressed: _toggleDirection,
            icon: Container(
              padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 3),
              decoration: BoxDecoration(
                border: Border.all(color: AppColors.beni),
                color: AppColors.beni.withValues(alpha: 0.08),
              ),
              child: Text(
                _koFirst ? tr('한→日') : tr('日→한'),
                style: const TextStyle(
                  fontSize: 11,
          ...
          IconButton(
            ...
            icon: Icon(
              ...
              color: _showHint ? AppColors.kinDeep : AppColors.sumiLight,
              size: 22,
            ),
          ),
  ...
  Widget _buildBody() {
    ...
    // 앞면/뒷면 내용 결정
    final showJaSide = _koFirst ? _flipped : !_flipped;
          ...
          child: Padding(
            ...
            child: GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: showJaSide ? AppColors.washiDeep : AppColors.washi,
                  border: Border.all(
                    color: switch (state) {
                      CardState.known => AppColors.matcha,
                      CardState.studying => AppColors.kinDeep,
                      CardState.unknown => AppColors.beni,
                    },
                    width: 1.6,
                  ),
                ),
                  ...
                  child: Column(
                    children: [
                      Row(
                        ...
                        children: [
                          Container(
                            ...
                            decoration: BoxDecoration(
                              color: isA ? AppColors.ai : AppColors.beni,
                      ...
                      if (showJaSide) ...[
                        ...
                        if (turn.kana != null && turn.kana != turn.ja)
                          Text(
                            turn.kana!,
                            textAlign: TextAlign.center,
                            style: const TextStyle(
                        ...
                        if (_koFirst && _showHint && turn.kana != null) ...[
                          const SizedBox(height: 16),
                          Container(
                            padding: const EdgeInsets.symmetric(
                                horizontal: 12, vertical: 7),
                            decoration: BoxDecoration(
                              color: AppColors.kin.withValues(alpha: 0.12),
                              border: Border.all(
                                  color: AppColors.kin.withValues(alpha: 0.7)),
                            ),
                            child: Text(
                              '💡 ${_hint(turn)}',
                              textAlign: TextAlign.center,
...
class _StateChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (label, color) = switch (state) {
      CardState.known => (tr('알아요'), AppColors.matcha),
      CardState.studying => (tr('공부중'), AppColors.kinDeep),
      CardState.unknown => (tr('몰라요'), AppColors.sumiLight),
    };
    return Container(
...
class _NavButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = enabled ? AppColors.sumi : AppColors.sumiLight.withValues(alpha: 0.5);
    final children = [
    ...
    return OutlinedButton(
      style: OutlinedButton.styleFrom(
        side: BorderSide(
            color: enabled
                ? AppColors.kin
                : AppColors.sumiLight.withValues(alpha: 0.3)),
